...
def main():
    ...
    scene_pbar = tqdm(vehicle_dirs, desc="Processing scenes", disable=args.verbose)
    ...
    for vehicle_dir in scene_pbar:
        ...
        for vehicle_id in vehicle_dirs:
            ...
            # core process
            ego_info, frames_processed, frames_skipped = process_scene_vehicle(vehicle_dir, output_root, args.verbose)
            all_ego_info.append(ego_info)
            total_frames_processed += frames_processed
            total_frames_skipped += frames_skipped
        
        # Add scene information (for pkl files) 
        scene_info = {
    ...
    with open(scene_infos_path, "wb") as f: # wb means write in binary mode
        pickle.dump(all_scene_infos, f) # pickle.dump() serializes the object and writes it to the file
